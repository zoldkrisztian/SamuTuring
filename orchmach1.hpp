...
public:
  ...
  void restart_step_by_step ( );
  int step_by_step ( int *center_of_tape , int noc );
...
template <int M> int TuringMachine< M >::step_by_step ( int *center_of_tape , int noc )
{
  ...
  //for ( long s ( 0 ); s < MAX_STEPS; ++s ) s->step_counter
  //  {
  ...
  for ( int i {-noc}; i<=noc; ++i )
    {
      center_of_tape[i+noc] = tape.get_tape ( tape.tapei + i );
    }
